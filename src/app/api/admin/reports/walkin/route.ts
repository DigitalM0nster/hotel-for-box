...
// API endpoint для получения отчёта по самовывозу (Walkin).
// Этот endpoint доступен только для администраторов.
// Возвращает заказы, которые клиенты могут забрать сами со склада
// (заказы с deliveryMethod === "warehouse").
export async function GET(request: Request) {
	try {
		...
		// Фильтр для самовывоза: способ доставки "warehouse"
		dateFilter["adressSnapshot.deliveryMethod"] = "warehouse";

		// 6. Получаем все заказы на самовывоз.
		...
		console.error("walkin report error", error);
		return NextResponse.json({ message: "Ошибка получения отчёта по самовывозу" }, { status: 500 });
	}
}
