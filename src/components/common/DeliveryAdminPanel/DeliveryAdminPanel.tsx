		...
		<div className="box w-full bg-f-white-100">
			<div className="flex flex-col gap-6">
				...
				<div className="flex flex-col gap-4">
					...
					<Input type="number" onChange={(e) => setWeight((state) => (Number(e.target.value) >= 0 ? e.target.value : state))} value={weight} afterText="кг" />
				</div>
				<div className="flex flex-col gap-4">
				...
				<div className="flex flex-col gap-4">
					...
					<Input type="text" value={h4b_us_id} onChange={(e) => setH4b_us_id(() => e.target.value)} />
				</div>

				<div className="flex flex-col gap-4">
					<div className="h5 text-f-blue-500">Стоимость услуг</div>
					<Input type="number" onChange={(e) => setOrder_coast((state) => (Number(e.target.value) >= 0 ? e.target.value : state))} value={order_coast} afterText="руб." />
				</div>
