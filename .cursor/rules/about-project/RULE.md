...
2. **Фаза Монголия → Россия (наша зона ответственности)**  
   - Посылка приходит на склад в Монголии:
     - админ ставит `originBranchId` (монгольский склад),
     - статус переводится в состояние прибытия (например, `Arrived`).
   - Далее:
     - замеряются вес/габариты (`weight`, `width_x`, `height_y`, `depth_z`),
     - считается `order_coast`,
     - админ выставляет счёт ⇒ статус `Shipping approved`,
     - пользователь оплачивает счёт (через платёжку) ⇒ `paid >= order_coast`, `paymentInfo.status = "paid"`, статус `Shipping paid`,
...
Статус: условно Created.
2. Админ считает финальную стоимость
...
3. Пользователь оплачивает весь счёт сразу
...
4. Дальше мы работаем только с уже оплаченным заказом
